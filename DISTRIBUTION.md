...
|--------|-----------|------------------|
| **`public/`** | **The finished website.** 300 pages of plain HTML, one CSS file, one JavaScript file, the fonts, icons and photography. This is what you upload to a web host. | Never edit by hand — it is regenerated |
| `content/` | Your articles, as plain text files. 38 stories plus the About, Contact, Terms, Privacy and Accessibility pages | Yes — this is where you write |
...
| `static/` | Pictures, fonts, favicons. 37 photographs under `static/images/photos/`, each filed at three sizes | Yes — put new pictures here |
...
|---|---|
| Total files | 819 |
| Source files (everything except `public/`) | 282 |
| Built pages in `public/` | 300 |
| Articles | 38 |
| Photographs | 37, each at three sizes |
| Videos referenced | 12 |
| Unzipped size | ~66 MB |
| Zip size | ~60 MB (or ~27 MB without `public/`) |
...
2. Drag `sportsone-world.zip` in. At **~60 MB** it uploads in seconds and is
   far below any Drive limit. It is too big to email — use Drive, or use the
   source-only zip (~27 MB), which rebuilds `public/` the first time you run
   Build-Website.bat.
3. **Do not let Drive unzip it.** If you use the Drive *app* and drop the
   *folder* rather than the zip, Drive syncs 819 separate files, which is slow
...
("Mark of the Web"). If you extract first, **every one of the 819 files carries
...
```powershell
(Get-ChildItem -Recurse -File).Count                       # expect 819
(Get-ChildItem public -Recurse -Filter *.html).Count       # expect 300
(Get-ChildItem static\images\photos -Filter *.jpg).Count   # expect 29
```

If the first number is far below 819, the extraction stopped early — usually
