...
## If you are a journalist, there are two ways to publish

**The tool.** Double-click **`Start-Publishing-Tool.bat`**. A form opens in your
browser with the real website beside it: type the story, add the photograph,
choose where on the site it goes and in which position, watch it appear in the
preview, and press Publish. Nothing is written by hand and the command line is
never opened.
Guide: **[docs/Publishing-Tool-Guide.docx](docs/Publishing-Tool-Guide.docx)**.

**The inbox.** Two files with matching names dropped into a folder — the
original route, unchanged, and still the quickest from a phone in a press box.
Guide: **[docs/Publishing-Guide.docx](docs/Publishing-Guide.docx)**.

Both write the same files and produce the same result. Use whichever suits the
moment. Everything below this line is for whoever maintains the site.

### The short version, using the inbox

Put your story and its photograph in **`inbox/`** with the **same file name**:
...
Then either double-click `Import-Articles.bat`, or upload the two files to the
`inbox` folder on github.com and press Commit. The picture is resized and
filed, the article is written, the front page is updated if you asked for it,
and the site republishes itself. Two or three minutes, start to finish.
...
| 8 | [docs/08-automated-morning-news.md](docs/08-automated-morning-news.md) | Research: automating a 6am sports-news brief with GitHub Actions, Cloudflare and a free LLM — what works, what does not, and what it costs |
| — | [docs/Publishing-Guide.docx](docs/Publishing-Guide.docx) | **The newsroom guide.** Give this to a journalist and nothing else |
...
├── scripts/
│   ├── cms/             THE PUBLISHING TOOL — runs on the desk, not the web
│   │   ├── server.py    Serves the form, runs Hugo, talks to git
│   │   ├── hugo_project.py  Reads and writes articles, placements, scores
│   │   └── ui/          One HTML page, one stylesheet, one script
│   ├── import-articles.py   Turns inbox/ into published articles
│   ├── make-tool-figures.py Re-shoots the tool guide's screenshots
│   ├── make-tool-guide.py   Rebuilds docs/Publishing-Tool-Guide.docx
